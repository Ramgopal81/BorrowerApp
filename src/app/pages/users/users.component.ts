...
import {
  ...
  UsersResponseModel,
} from 'src/app/models/api-responseModel';
          ...
          if (response) {
            ...
            if (response.user) {
              response.user.forEach((element: any) => {
                if (element.user_id) {
                  element.user_id = this.apiService.decryptionAES(
                    element.user_id
                  );
                }
                if (element.firstname) {
                  element.firstname = this.apiService.decryptionAES(
                    element.firstname
                  );
                }
                if (element.lastname) {
                  element.lastname = this.apiService.decryptionAES(
                    element.lastname
                  );
                }
                if (element.email_id) {
                  element.email_id = this.apiService.decryptionAES(
                    element.email_id
                  );
                }
                if (element.mobile_no) {
                  element.mobile_no = this.apiService.decryptionAES(
                    element.mobile_no
                  );
                }
                if (element.companyName) {
                  element.companyName = this.apiService.decryptionAES(
                    element.companyName
                  );
                }
  ...
  filterDatatable(key: string) {
    ...
    this.userData = this.tempArray.filter((item: any) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (
          (item?.firstname &&
            item?.firstname.toString().toLowerCase().indexOf(filterKey) !==
              -1) ||
          !filterKey
        ) {
